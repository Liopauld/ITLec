...
import { ArrowLeft, Plus, Trash2, Save, BookOpen, Gamepad2, GraduationCap, Sparkles, Edit3 } from 'lucide-react';
...
export default function EditTrack() {
  ...
  useEffect(() => {
    ...
    // Fetch the track to edit
    fetch(`http://localhost:4000/tracks/${id}`)
      .then(res => res.json())
      .then(data => {
      ...
      .catch(err => {
        ...
        alert('Failed to load track for editing');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.category && formData.difficulty !== 'beginner') {
      ...
      prerequisites = existingTracks
        .filter(track => track.category === formData.category && track.difficulty === lowerDifficulty && track.id !== id)
        .map(track => track.id);
    }
    ...
    try {
      const response = await fetch(`http://localhost:4000/tracks/${id}`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        ...
        alert('Failed to update track: ' + error.error);
      }
    } catch (err) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 text-lg">Loading track for editing...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 py-8">
      <div className="container mx-auto px-4 max-w-5xl">
        ...
        {/* Header Section */}
        <div className="bg-gradient-to-r from-green-600 via-emerald-600 to-teal-600 rounded-3xl p-8 shadow-2xl text-white mb-8">
          <div className="flex items-center gap-4 mb-4">
            <div className="bg-white/20 backdrop-blur-sm rounded-xl p-3">
              <Edit3 className="w-8 h-8" />
            </div>
            <div>
              <h1 className="text-4xl font-bold mb-2">Edit Learning Track</h1>
              <p className="text-green-100">Update your learning track with new content and modules</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-8 shadow-xl border border-gray-100">
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            {/* Basic Info */}
            <div className="space-y-6 bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl p-6 border-2 border-green-100">
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between bg-gradient-to-r from-blue-50 to-purple-50 rounded-2xl p-6 border-2 border-blue-200">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">Update your curriculum with structured modules</p>
                  </div>
                </div>
                <button
                  type="button"
                  onClick={addModule}
                  className="flex items-center gap-2 bg-gradient-to-r from-blue-600 to-purple-600 text-white px-6 py-3 rounded-xl hover:from-blue-700 hover:to-purple-700 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105 font-semibold"
                >
            ...
            <div className="flex justify-between items-center bg-gradient-to-r from-gray-50 to-gray-100 rounded-2xl p-6 border-2 border-gray-200">
              <div>
                ...
                <p className="text-xs text-gray-500">Review your changes before updating</p>
              </div>
              <button
                type="submit"
                disabled={saving}
                className="flex items-center gap-3 bg-gradient-to-r from-green-600 via-emerald-600 to-teal-600 text-white px-8 py-4 rounded-xl font-bold hover:from-green-700 hover:via-emerald-700 hover:to-teal-700 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed shadow-xl hover:shadow-2xl transform hover:scale-105"
              >
                <Save className="w-5 h-5" />
                {saving ? (
                  <>
                    <span className="animate-pulse">Updating Track...</span>
                  </>
                ) : (
                  'Update Track'
                )}
